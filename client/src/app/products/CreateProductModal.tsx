...
const CreateProductModal = ({isOpen, onClose, onCreate}: CreateProductModalProps) => {
    ...
    const labelCssStyles ='block text-sm fontmedium text-gray-700';
    const inputCssStyles = 'block w-full mb-2 p-2 border-gray-500 border-2 rounded-md'

  return (
    <div className='fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-20'>
        <div className='relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white '>
            <Header name='Create new Peroduct' />
            <form onSubmit={handleSubmit} className='mt-5'>
                ...
                <button type='button' className='
                 ml-2 px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-700' onClick={onClose}>
                    Cancel
                </button>
            </form>
        </div>
